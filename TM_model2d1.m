function [wavefield_gradient,gather] = TM_model2d1(ep,mu,sig,xprop,zprop,srcloc,recloc,srcpulse,t,npml,outstep,plotopt,x,z,dt_wf,nt_wf)
% TM_model2d.m
% 
...
nx = (length(xprop)+1)/2;           % maximum number of field nodes in the x-direction  nx=222
nz = (length(zprop)+1)/2;           % maximum number of field nodes in the z-direction  nz=132
dx = 2*(xprop(2)-xprop(1));         % electric and magnetic field spatial discretization in x (m) dx=0.1
dz = 2*(zprop(2)-zprop(1));         % electric and magnetic field spatial discretization in z (m) dz=0.1
...
nsrc = size(srcloc,1);                          % number of sources nsrc=1
nrec = size(recloc,1);                          % number of receivers nrec=19
for s=1:nsrc                                   
...
wavefield_gradient.wavefield=zeros(nx-2*npml-1,nz-2*npml-1,nt_wf);
% ------------------------------------------------------------------------------------------------
...
% loop over number of sources
 for s=1:nsrc
    ...
    PHz = zeros(nx,nz-1);           % psi_Hz (for PML)
      ...
      %wavefield = zeros(fix((numit-1)/outstep(1))+1, fix((length(Ey(:,1))-1)/outstep(3))+1, fix((length(Ey(1,:))-1)/outstep(2))+1);
    % time stepping loop
    for it=1:numit
...
%    x2 = min(x):dx:max(x);
%    z2 = min(z):dx:max(z);
%    Ey1=Ey(npml+1:end-npml-1,npml+1:end-npml-1); % huainan 
%    Ey1 = gridinterp(Ey1,x2,z2,x,z,'nearest');
 
    Ey1=Ey(npml+1:end-npml,npml+1:end-npml);
    Eyx=Eydiffx(npml+1:end-npml,npml+1:end-npml);
    Eyz=Eydiffz(npml+1:end-npml,npml+1:end-npml);
     in_wf=floor(dt_wf/dt);
     it_record=1+floor((it-1)/in_wf);
     wavefield_gradient.fux(:,:,it_record) = Eyx(1:outstep(3):end,1:outstep(2):end);
     wavefield_gradient.wavefield(:,:,it_record) = Ey1(1:outstep(3):end,1:outstep(2):end);
        
%         if mod(it-1,outstep)==0
             tout((it-1)/outstep(1)+1) = t(it);
%              wavefield((it-1)/outstep(1)+1,:,:) = Ey1(1:outstep(3):end,1:outstep(2):end);
%         end     
          for r=1:nrec
                gather(it,r,s) = Ey(reci(r),recj(r));
            end
    end
end
